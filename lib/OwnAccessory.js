...
class OwnBlindAccessory extends OwnAccessory {
    ...
    startMoveTracking() {
        ...
        this.moveTrackingTimeout = undefined;
        this._moveRetries = (this._moveRetries || 0) + 1;
        if (this._moveRetries > 30) {
...
class OwnScenarioAccessory extends OwnAccessory {
    ...
    destroy() { clearTimeout(this._resetTimeout); }
}
...
class OwnEnergyAccessory extends OwnAccessory {
    constructor(platform, accessory, config) {
        ...
        this._pollInterval = setInterval(() => {
            if (this.controller._commandQueue.length < 10) this.updateStatus();
        }, 30000);
    }
    ...
    onData(packet) {
        ...
        if (extract) {
            ...
            this.log.debug("[" + this.id + "] Energy: ignoring packet %s", packet);
        }
    }
}
