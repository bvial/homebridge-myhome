...
class OwnBlindAccessory extends OwnAccessory {
    ...
    updateStatus() {
        ...
        if (!this.initStartPosition ) {
            this.log.info("[" + this.id + "] Initialistion phase of blind : reset postion to 0 and send move down  ");
    ...
    onData(packet) {
        ...
        if (extract) {
            ...
            this.log.info("[" + this.id + "] receive state  dir : " + direction + " position:" + this.position + " target:" + this.target);

            if ( this.commandisPending() && this.expectedState == this.state) {
                this.log.info("[" + this.id + "] the expected state:" + this.state + "  reached: " + this.expectedState );
