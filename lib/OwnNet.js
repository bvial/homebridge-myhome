...
function calcPass(pass, nonce) {
    ...
    if (isNaN(password)) throw new Error('OWN password must be a number, got: ' + pass);
    for (var c of nonce) {
        if (c != '0') {
...
class OwnConnection extends events.EventEmitter {
    ...
    connect() {
        ...
        this.conn.on('error', (_error) => {
            ...
            this.state = STATE.UNCONNECTED;
            // Node.js fires 'close' after 'error' — no need to emit here
        });
    ...
    onData(data) {
        ...
        /* handle the fact that more that one packet can come in at the same time,
           and that TCP may deliver partial packets */
        while (this._buf.length > 0) {
            var m = this._buf.match(/(\*.+?##)([\s\S]*)/);
            if (!m) {
                if (this._buf.length > 4096) {
                    this.log.warn('conn:%s buffer overflow (%d bytes), discarding', this.id, this._buf.length);
                    this._buf = '';
                }
                break;
            }
...
class OwnMonitor extends events.EventEmitter {
    ...
    connect() {
        ...
        this.connection.on('auth-failed', function () {
    ...
    restartConnection() {
        ...
        if (this.nbCheck < 3 ) {
          ...
          const delaySec = Math.min(this.reconnectSeconds * this._reconnectAttempts, 300);
          this.client.log.error("Monitor connection is dead, reconnecting in %ds (attempt #%d)", delaySec, this._reconnectAttempts);
          this._reconnectTimeout = setTimeout(() => this.connect(), delaySec * 1000);
        }
    }
}
...
class OwnClient extends events.EventEmitter {
    ...
    sendCommand(params) {
        if (this._commandQueue.length >= 50) {
            this.log.warn('Command queue full (%d), dropping: %s', this._commandQueue.length, params.command);
            return;
        }
        this._commandQueue.push(params);
        this._processQueue();
    }

    _processQueue() {
        while (this._activeCommands < this._maxConcurrent && this._commandQueue.length > 0) {
            this._activeCommands++;
            this._executeCommand(this._commandQueue.shift());
        }
    }

    _releaseSlot() {
        if (this._activeCommands > 0) this._activeCommands--;
        this._processQueue();
    }

    _executeCommand(params) {
        var released = false;
        var releaseSlot = (failed) => {
            if (!released) {
                released = true;
        ...
        commandconn.on('close', () => releaseSlot(true));
        commandconn.connect();
