...
class OwnBlindAccessory extends OwnAccessory {
    ...
    getState(callback) {
        ...
        this.controler.sendCommand({ command: '*#2*' + this.id + '##', log: this.log });
    ...
    updateStatus() {
        ...
        if (!this.initStartPosition ) {
            this.log.info("[" + this.id + "] Blind reset postion to 0 and send move down  ");
    ...
    startTimerCommand() {
        ...
        this.packetTimeout = setTimeout(this.endTimercommand.bind(this), 2000);
    }
    ...
    onData(packet) {
        ...
        if (extract) {
            ...
            this.log.info("[" + this.id + "] receive state  dir : " + direction + " pos:" + this.position + " tag:" + this.target);

            if (this.expectedState == this.state && this.commandisPending()) {
                this.log.info("[" + this.id + "] the expected state is reached: " + this.state );
    ...
    stopMoveTraking() {
        ...
        if (this.state != this.expectedState) {
            ...
            this.log.error("[" + this.id + "] Blind command confirmation didn't receive => force state to : "+ this.expectedState);
            this.updateStatus();
        }
        this.endTimercommand();
    }
